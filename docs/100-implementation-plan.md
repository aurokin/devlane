...
- explicit Phase 1 failure semantics for `prepare`: validate everything that can fail before writing, stage repo-local writes to temp files in the destination directories, promote them in deterministic order via atomic rename where possible, and report partial local state clearly if a late promotion fails. Catalog-coupled publish semantics for `prepare` / `reassign` land in Phase 2
...
- catalog-coupled publish semantics for `prepare` and the write half of `reassign`: preflight repo-local failures first, compute mutations under lock, stage and promote repo-local writes deterministically, publish `catalog.json` only after required promotions succeed, and report partial local state clearly when a late local promotion fails
